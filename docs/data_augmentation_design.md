...
Figure 4: Image visualizing the data augmentation process.
...
11. A loss multiplier array is generated. This array normalizes the loss by the interocular distance of the face. This way the network trains as fast on large faces as it does on small faces. The array also reduces the loss for landmarks that lie outside the bounding box. The idea behind this is that the neural network may train badly by training on landmarks it can not see. Reducing this loss may not pan out well as training on landmarks the network can not see also has a regularizing effect.
12. The image, landmarks and loss multipliers are written to arrays of images, landmarks and loss multipliers.
<a id="13-and-14"></a>
...
The attribute flags (Large pose, Expression, Illumination, Makeup, Occlusion, Blur) are currently not used in the creation of the dataset, but can be used to improve the performance of the model.
